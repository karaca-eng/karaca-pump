...
class MarketRadar:
    ...
    def process_ticker(self, data):
        ...
        with self.lock:
            ...
            for item in data:
                ...
                # Kapasiteyi 1200 yaptık (~20 dakika veri biriktirir)
    ...
    def check_logic(self, symbol, now):
        ...
        # 1. KATMAN: FLASH ATTACK (Anlık veriye dayalı)
        ...
        # 2. KATMAN: CONFIRMED TREND (Yerel hafızaya dayalı)
        # Sinyal tetiklenmesi için en az 15 dakikalık veri birikmiş olmalı (data_age_seconds)
        if vol_3m >= MIN_VOL_3M and abs(c3) >= MIN_CHG_3M:
            if data_age_seconds >= LONG_WINDOW:
                past_15m = hist[0] # Hafızadaki en eski veri (~15-20 dk öncesi)
...
# --- UI ---
st.set_page_config(layout="wide", page_title="SinyalEngineer Global Radar")
...
h1.caption("Global Cloud Optimized (No External REST API dependency)")
...
while True:
    ...
    with placeholder_main.container():
        with radar.lock:
            ...
            if display_data:
                html = "<table><tr><th>Time</th><th>Symbol (4H ↑/↓)</th><th>Price</th><th>Mtm.</th><th>15m Ref</th><th>Vol</th><th>Status</th><th>Type</th></tr>"
                ...
                # Isınma süresi uyarısı
                st.info("Scanning Market... (Wait 15 mins for Trend Confirmation signals to start appearing online) 🔍")
    time.sleep(1)
